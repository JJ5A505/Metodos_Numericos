...
public class MatrixInputWindow2 extends Stage {
    ...
    public MatrixInputWindow2(int rows, int columns) {
        this.rows = rows;
        ...
        // Agregar el campo de entrada para el error porcentual debajo de la matriz
        errorField.setPromptText("Error porcentual");
        TextFormatter<Double> errorFormatter = new TextFormatter<>(doubleConverter);
        errorField.setTextFormatter(errorFormatter);
        gridPane.add(errorField, 0, rows);

        gridPane.add(resolver, 0, rows + 1); // Agregar el botón "Resolver" debajo del campo de error
        ...
        resolver.setOnAction(event -> {
            ...
            SolutionWindow2 solutionWindow = new SolutionWindow2(solutionMatrix, error);
